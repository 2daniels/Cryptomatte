...
def setup_cryptomatte_ui():
    if nuke.GUI:
        ...
        menu.addCommand("CryptoGrade", "import cryptomatte_utilities as cu; cu.cryptograde_create_gizmo();")
        menu.addCommand("CryptoKill", "import cryptomatte_utilities as cu; cu.cryptokill_create_gizmo();")
...
def setup_cryptomatte():
    ...
    nuke.addOnCreate(lambda: encryptomatte_on_create_event(
        nuke.thisNode(), nuke.thisKnob()), nodeClass='Encryptomatte')
    nuke.addKnobChanged(lambda: cryptomatte_knob_changed_event(
        nuke.thisNode(), nuke.thisKnob()), nodeClass='CryptoGrade')
    nuke.addKnobChanged(lambda: cryptomatte_knob_changed_event(
        nuke.thisNode(), nuke.thisKnob()), nodeClass='CryptoKill')
...
class CryptomatteInfo(object):
    def __init__(self, node_in):
        ...
        if self.nuke_node.Class() in ["Cryptomatte", "Encryptomatte", "CryptoGrade", "CryptoKill"]:
...
def cryptograde_create_gizmo():
    return nuke.createNode("CryptoGrade")

def cryptokill_create_gizmo():
    return nuke.createNode("CryptoKill")
